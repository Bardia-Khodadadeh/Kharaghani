...
std::vector<std::vector<Detection> > Detector_TorchScript_Yolo::run(std::vector<cv::Mat> &images, int cup)
{
    std::vector<cv::cuda::GpuMat> gImages;
    for (auto &img : images) {
        cv::cuda::GpuMat gImg;
        gImg.upload(img);
        gImages.push_back(gImg);
    }

    int batchSize = gImages.size();
    try {
        auto t0 = std::chrono::high_resolution_clock::now();
        std::vector<cv::Size> orgImagesSize;
        std::vector<PadIno> pads_info = LetterboxImage(gImages, orgImagesSize);
        torch::Tensor tensor_batch = convertToTensor(gImages);
        ...
        //qDebug () << "cup______" << cup;
        qDebug () <<" ------- Process time: Total" << dt;
...
std::vector<PadIno> Detector_TorchScript_Yolo::LetterboxImage(std::vector<cv::cuda::GpuMat> &images, std::vector<cv::Size> &orgImagesSize)
{
    ...
    for (int i = 0; i < static_cast<int>(images.size()); ++i){
        ...
        PadIno _pad;
        _pad.scale = std::min((float)images[i].cols / images[i].cols, (float)images[i].rows / images[i].rows);
        ...
        cv::cuda::copyMakeBorder(images[i], images[i], _pad.top, _pad.down, _pad.left, _pad.right, cv::BORDER_CONSTANT, cv::Scalar(0, 0, 0));
...
at::Tensor Detector_TorchScript_Yolo::convertToTensor(std::vector<cv::cuda::GpuMat> &images)
{
    for (auto &gimg : images) {
        cv::cuda::cvtColor(gimg, gimg, cv::COLOR_BGR2RGB);
        gimg.convertTo(gimg, CV_32FC3, 1.0f / 255.0f); // normalization
    }
